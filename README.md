...
이 저장소는 설치 파일과 자동 업데이트 파일만 올리는 배포용 저장소입니다.
프로그램 소스 코드는 포함되어 있지 않습니다.

## 다운로드

정식 최신 릴리즈는 아래 Releases 페이지에서 받을 수 있습니다.
...
새 업데이트가 있으면 앱 안에서 업데이트 안내가 표시됩니다.
`다운로드`를 누르면 업데이트 파일을 내려받고, 다운로드가 끝난 뒤 `재시작 후 설치`를 누르면 새 버전으로 바뀝니다.
